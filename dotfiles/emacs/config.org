#+TITLE: Emacs Configuration for Markdown & Terminal
#+AUTHOR: Beginner Setup

* Keybindings (Evil Mode)

  Since we are coming from Neovim, we implement evil-mode to preserve all Vim muscle memory inside the terminal wrapper.
...
#+end_src
  ...
  * Markdown Editing & Syntax Optimization

  This optimizes 'markdown-mode' for the 'st' terminal emulator. It hides raw markdown markers (like `**`) while typing unless your cursor sits on them, keeping the code clean. Centering and automatic wrapping are omitted.
...
This section configures universal editor behaviors, including Neovim-style relative line numbering.
